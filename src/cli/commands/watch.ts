...
export function registerWatch(program: Command): void {
  program
    .command('watch <path>')
    .description('Watch a directory and auto-mine changed files into the palace')
    .option('--palace <path>', 'Palace path override')
    .option('--wing <name>', 'Wing name (default: directory name)')
    .option('--debounce <ms>', `Debounce delay in ms (default: ${DEFAULT_DEBOUNCE_MS})`)
    .option('--quiet', 'Suppress per-file log output')
    .action(
      async (
        targetPath: string,
        opts: {
